...
class GUI():
    def __init__(self,master):
        ...
        self.label = Label(text=";PASSWORD GENERATOR:", anchor=N, fg='darkblue', bg='#FF8000', font='aria1 20 bold underline')
        ...
        self.blank_label1.grid(row=2, column=0, columnspan=2)

        self.blank_label2 = Label(text="")
        self.blank_label2.grid(row=3, column=0, columnspan=2)

        self.user = Label(text="Enter User Name:", font='time 15 bold', bg='#FF8000', fg='darkblue')
        ...
        self.length = Label(text="Enter Password Length: ", font='time 15 bold', bg='#FF8000', fg='darkblue')
        self.length.grid(row=6, column=0)

        self.length_textfield = Entry(textvariable=self.n_passwordlen, font='time 15', bd=6, relief='ridge')
        self.length_textfield.grid(row=6, column=1)

        self.blank_label4 = Label(text="")
        self.blank_label4.grid(row=7, column=0)

        self.generated_password = Label(text="Generated Password: ", font='time 15 bold', bg='#FF8000', fg='darkblue')
        self.generated_password.grid(row=8, column=0)

        self.generated_password_textfield = Entry(textvariable=self.n_generatedpassword, font='time 15', bd=6, relief='ridge', fg='#DC143C')
        self.generated_password_textfield.grid(row=8, column=0)
        ...
        self.generate = Button(text="GENERATE PASSWORD", bd=3, relief='solid', padx=1, pady=1, font='Verdana", 15 bold', fg='#68228B', bg='#BCEE68', command=self.generatedpassword)
        ...
        def generate_pass(self):
            ...
            leng = self.length_textfields.get()

            with sqlite3.connect("users.db") as db:
                cursor = db.cursor()

            if name=="":
                messagebox.showerror("Error", "Name must be a empty")
                return
            ...
            if length<6:
                messagebox.showerror("Error", "Password must be atleast 6 charachters long")
